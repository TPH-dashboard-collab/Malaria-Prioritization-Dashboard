...
# Calculates impacts for ALL plans at startup
# ==============================================================================
...
per_interv_impact <- function(df,
  ...
  # Só remove a coluna se existir — evita warning quando counterfactual não encontrou linha
  impact_col <- paste0("IMPACT_", indicator)
  cols_to_remove <- intersect(impact_col, names(result))
  if (length(cols_to_remove) > 0) {
    result <- result[, (cols_to_remove) := NULL]
  }
...
# Get strata for all plans at once
df1 |>
...
# Add plan column back (lost in per_interv_impact)
impacts_all <- impacts_all |>
  left_join(
    strata_all |> select(admin_2, age_group, scenario_name, plan) |> distinct(),
    by = c("admin_2", "age_group", "scenario_name")
  )

cat("\n✓ Impacts calculated:", nrow(impacts_all), "rows\n")
cat("✓ Plans in impacts:", paste(sort(unique(impacts_all$plan)), collapse = ", "), "\n")

# ==============================================================================
# STEP 4: CALCULATE avg_impact (with plan)
# ==============================================================================
...
# ==============================================================================
# STEP 5: CALCULATE QUANTILES (with plan)
# ==============================================================================

cat("Calculating quantiles...\n")

quantiles_all <- avg_impact_all |>
...
cat("✓ Quantiles calculated:", nrow(quantiles_all), "rows\n")

# ==============================================================================
# STEP 6: CALCULATE RANKINGS (with plan)
# ==============================================================================

cat("Calculating rankings...\n")

rankings_all <- avg_impact_all |>
  group_by(plan, intervention, admin_2) |>
  summarise(
...
cat("✓ Rankings calculated:", nrow(rankings_all), "rows\n")
...
cat("═══════════════════════════════════════════════════════\n")
cat("✓ global.R complete — all plans ready\n")
